...
- **사진은 태블릿 기본 카메라로 찍는다.** 앱 안에서 카메라를 열지 않는다 —
  그렇게 찍은 사진은 앱으로만 전달되고 기기 갤러리에 남지 않기 때문이다(브라우저 제약).
  찍은 뒤 앱에서 `🖼 사진 선택` 으로 불러온다.
- 대표 이미지는 불러오는 즉시 **자르기 화면**이 뜬다.
  모니터 두 대를 한 장에 찍었으면 「왼쪽 절반 / 오른쪽 절반」 으로 한 대만 떼어 A·B 탭에 각각 넣는다.
  (같은 사진을 두 번 불러와 각각 자르면 된다. 원본은 갤러리에 그대로 있다)
  한 대뿐이면 「자르지 않고 사용」.
- 대표 이미지의 **⌁** 버튼 → 모니터 오버레이에서 캠 롤·클립을 읽어 그 캠에 채운다.
  판독값은 확인창을 거친다.
- 이미 넣은 사진도 타일의 **✂** 로 다시 자를 수 있다.
