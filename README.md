...
- 씬 안에서 카메라를 **A / B / C / D 탭**으로 나눈다.
  - **캠별(독립)** : 대표 이미지 · 촬영 유닛 · 캠 롤 · 클립 · 현장 사진 — 스키마에 `cam:true`,
    `rec.cams.<탭>.<키>` 에 저장된다
  - **씬 공통(공유)** : 에피소드 · 씬 · 로케이션 · INT/EXT · 시제 · 벤더 · 촬영일시 ·
    에셋 · HDRI · 노트 — 탭을 바꿔도 같은 값
  - 값이 들어 있는 탭은 테두리로 표시되고, 탭 아래에 `A027 C002` 요약이 뜬다
- **「📷 모니터 촬영」** 이 주 입력 경로다.
  모니터를 찍으면 캠 롤(`A027`)과 클립(`C001`)을 읽어 **그 캠 탭**에 채운다.
  대표 이미지가 비어 있으면 거기에, 아니면 그 캠의 현장 사진 빈 칸에 들어간다(덮어쓰지 않음).
  어느 탭에 넣을지는 판독 확인창에서 최종 확인한다.
- `+ 새 씬` → 직전 씬의 EP/유닛/INT-EXT/시제/로케이션/벤더를 상속하고 촬영일·시각 자동 기록.
  ID 는 `<프로젝트약어>-YYYYMMDD-HHMMSS-XXXX`
- 모든 입력은 **0.5초 자동 저장** (저장 버튼 없음, ID 옆 점이 초록으로 깜빡임)
- 씬의 로케이션은 자유 입력이 아니라 **Location 레코드 연결**이다. 에셋 · HDRI 도 드롭다운으로 고른다.
- 이미지는 대표 1280px / 현장 1920px / 모니터·HDRI 2560px 로 JPEG 재압축
- 목록의 대표 이미지는 **A → B → C → D** 순으로 훑어 먼저 나오는 것을 쓴다 (A 가 있으면 무조건 A)

> `cuts` 스토어(옛 컷/테이크 구조)는 **지우지 않았다.** 화면에서 안 보일 뿐 백업 JSON 에 그대로 남는다.
> 대신 VFX 작업 타입 · 작업 요소별 물량 집계는 근거가 사라져 Overview 에서 빠졌다.
